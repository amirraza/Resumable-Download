package com.home.amirraza.resumabledownloadsample;
...
public class MainActivity extends AppCompatActivity {
    ...
    String downloadFileUrl = "https://upload.wikimedia.org/wikipedia/commons/b/b8/ESO_Very_Large_Telescope.jpg";
    private long myDownloadReference;
    private BroadcastReceiver receiverDownloadComplete;
    private BroadcastReceiver receiverNotificationClicked;
    private BroadcastReceiver receiverDownloadingInterrupted;
...
//                        do something with the download file
        ...
        //filter for downloading interruption
        IntentFilter intentFilter1 = new IntentFilter(WifiManager.WIFI_STATE_CHANGED_ACTION);
        receiverDownloadingInterrupted = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
//                WifiManager wifiManager = (WifiManager) getSystemService(Context.WIFI_SERVICE);
                ...
                if(state == WifiManager.WIFI_STATE_ENABLED){
                    Toast.makeText(context,"Enabled WIFI",Toast.LENGTH_SHORT).show();
                }
                else if(state == WifiManager.WIFI_STATE_DISABLED){
                    Toast.makeText(context,"Disabled WIFI",Toast.LENGTH_SHORT).show();
                }
...
//                    Log.d("TAG keySet",intent.getExtras().keySet()+"");
//
//                Bundle bundle = intent.getExtras();
//                for(String key : bundle.keySet()){
//                    Log.d("TAG key",key);
//                    Log.d("TAG key get",""+bundle.get(key));
//
//                }
...
//                    Log.d("TAG","intentAction["+i+"]"+intent.getAction());
//                    Log.d("TAG","keySet["+i+"]"+intent.getExtras().keySet());
//
//                    Log.d("TAG", "netAvail[" + i + "]" + networkInfo[i].getState());
//                    Log.d("TAG","netAvail "+networkInfo[i].isAvailable());
//                    Log.d("TAG","netConnected "+networkInfo[i].isConnected());
//                    Log.d("TAG", "netConnectedORConnecting " + networkInfo[i].isConnectedOrConnecting());
//                    Log.d("TAG","netFailOver "+networkInfo[i].isFailover());
//                    Log.d("TAG","netRoam "+networkInfo[i].isRoaming());
//                }
            }
        };

        registerReceiver(receiverDownloadingInterrupted,intentFilter1);
...
//                            startActivity(intentDisplay);
    ...
    protected void onPause() {
        ...
        unregisterReceiver(receiverDownloadingInterrupted);
    }
}
